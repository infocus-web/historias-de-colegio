...
type AlumnoRegistrado = { id: string; nombre: string; grado: string; division: string | null; turno: string | null };
...
export default function Flow({ colegio, evento, fotos }: { colegio: Colegio; evento: Evento; fotos: Foto[] }) {
  ...
  // Detecta si ya se etiquetó al menos una foto de cada tipo para este evento — ya sea por
  // curso (grado, el método actual) o, en fotos más viejas, por alumno puntual (el método
  // anterior). Mientras el fotógrafo no haya asignado nada todavía, no filtramos nada — así
  // el evento demo (con fotos sin etiquetar) sigue mostrando todo, como antes.
  const fotosIndividualTaggedExist = useMemo(
    () => fotos.some((f) => f.categoria === "individual" && (f.grado || f.alumno_id)),
    [fotos]
  );
  const fotosDocenteTaggedExist = useMemo(
    () => fotos.some((f) => f.categoria === "docente" && (f.grado || f.alumno_id)),
    [fotos]
  );
  ...
  // Una foto es del curso del alumno/a actual si coinciden grado + división + turno. División
  // y turno son opcionales: una foto sin división "matchea" con un alumno sin división, y así
  // con turno — no hace falta que ambos estén cargados para que funcione.
  function esDelCursoDe(f: Foto, alumno: AlumnoRegistrado) {
    return (
      f.grado === alumno.grado &&
      (f.division ?? "") === (alumno.division ?? "") &&
      (f.turno ?? "") === (alumno.turno ?? "")
    );
  }

  // Fotos que le corresponden al alumno/a actual: si ya hay etiquetado para esa categoría,
  // filtra por curso (grado+división+turno) — o, para fotos etiquetadas con el método anterior,
  // por alumno puntual. Si no hay nada etiquetado todavía, muestra todas las fotos de la
  // categoría (comportamiento de antes, para no romper el demo).
  const fotosDelAlumno = useMemo(() => {
    if (!alumnoActual) return fotos;
    return fotos.filter((f) => {
      if (f.categoria === "individual") {
        if (!fotosIndividualTaggedExist) return true;
        return f.grado ? esDelCursoDe(f, alumnoActual) : f.alumno_id === alumnoActual.id;
      }
      if (f.categoria === "docente") {
        if (!fotosDocenteTaggedExist) return true;
        return f.grado ? esDelCursoDe(f, alumnoActual) : f.alumno_id === alumnoActual.id;
      }
      if (f.categoria === "grupal") {
        if (!fotosGrupalTaggedExist) return true;
        return esDelCursoDe(f, alumnoActual);
      }
      // "varias": no se etiquetan por alumno/curso — se muestran a todas las familias del evento.
      return true;
    });
